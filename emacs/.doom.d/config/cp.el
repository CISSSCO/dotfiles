...
(defun cisco/problem ()
  ...
  (let* (
         ...
         (org-base "~/git/org/")
         (roam-base "~/git/org/roam/")
         (problem-base (concat org-base "problem/"))

         ;; timestamp
         (timestamp (format-time-string "%Y-%m-%d-%H%M"))

         ;; problem folder (code files)
         (problem-dir (concat problem-base timestamp "/"))
         (main-file (concat problem-dir "main.cpp"))
         (input-file (concat problem-dir "input.txt"))
         (output-file (concat problem-dir "output.txt"))

         ;; roam files
         (problem-node (concat roam-base "problem/" timestamp ".org"))
         (central-node (concat roam-base "problem/problem.org"))

         problem-id central-id
         )

    ;; ensure dirs
    (make-directory problem-dir t)
    (make-directory (file-name-directory problem-node) t)

    ;; create central problem node if not exists
    (unless (file-exists-p central-node)
      (with-temp-file central-node
        (insert "#+title: Problem\n#+filetags: problem\n\n* All Problems\n")))
    ...
    ;; create symlink inside problem folder → points to roam node
    (let ((symlink-path (concat problem-dir "problem.org")))
    (unless (file-exists-p symlink-path)
        (make-symbolic-link (expand-file-name problem-node) symlink-path)))

    ;; create problem node
    (unless (file-exists-p problem-node)
      (with-temp-file problem-node
        (insert (format "#+title: Problem %s\n#+filetags: problem\n\n* Files\n- [[file:%s][Open Folder]]\n"
                        timestamp problem-dir))))

    ;; ensure problem node has ID
    (with-current-buffer (find-file-noselect problem-node)
      (org-id-get-create)
      (setq problem-id (org-id-get))
      (save-buffer))
